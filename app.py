...
# Instantiate cyberbullying models
...
def get_sentiment_prediction(text):
    # Get emsemble result for sentiment models
    prediction = get_ensemble_prediction(sentiment_models, text)
    # For displaying as a percentage, multiply and round the probability
...
def get_cyberbullying_prediction(text):
    # Get emsemble result for cyberbullying models
...
# Handler for form submission, returns layout template with results container
...
def result():
    ...
    # Return the view with the prediction results
